...
<head>
    ...
    <link rel="stylesheet" href="style/courrierEntrantCSS.css">
        ...
        <div class="container">
            ...
            <table class='table table-striped table-hover mt-5'>
                <tr>
                    <td colspan='7' style="background-color: #353b49;">
                        <div class="d-flex" style="position:relative;">
                            <button class="btn btn-ajouter"><a href="/ajouter" style="text-decoration:none;color:#000;font-weight:500;"><i class="fa-solid fa-circle-plus"></i>إضافة بريد</a></button>
                            <form action="{{route('courrier.search')}}" method="post">
                                @csrf
                ...
                <tr>
                    ...
                    <td>{{$donnee->NumeroInscriptionAcademie}}</td>
                    <td>{{$donnee->DateInscriptionAcademie}}</td>
                    <td>{{$donnee->Expediteur}}</td>
                    <td style="max-width: 200px; word-wrap: break-word;">{{$donnee->SujetCorrespondance}}</td>
                    <td>{{$donnee->Statut}}</td>
                    <td>
                        <a href="/archiverEntrant/{{$donnee->id}}" title='أرشفة'><i class="fa-solid fa-inbox"></i></a> |<a href="/modifier/{{$donnee->id}}" title='تغيير'><i class="fa-solid fa-pen-to-square"></i></a><br>
                        <a href="/voir/{{$donnee->id}}" title='إظهار'> <i class="fa-solid fa-eye"></i></a> | <button id="openModalBtn{{$donnee->id}}" title='مسح'><i class="fa-solid fa-trash"></i></button>
                    </td>
                </tr>
                <!-- The Modal -->
                <div id="myModal{{$donnee->id}}" class="modal">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="btn-container">
                            <form method="POST" action="{{ route('delete-Incoming', ['id' => $donnee->id]) }}">
                                @csrf
                                @method('DELETE')
                                <button class='btn bg-danger text-white fw-bold' type="submit">حـــــذف</button>
                            </form>
                            <!-- Button to close the modal -->
                            <a class='btn' style='border:1px solid silver;font-weight:700;' href="/courrierEntrant">رجــــــوع</a>
                        </div>
                    </div>
                </div>
            ...
            <button class="btn btn-ajouter">
                <a href="/ajouter" style="text-decoration:none;color:#000;font-weight:500;">
